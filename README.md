...
Switching to the officially published **int8-quantized ONNX** weights and calling `onnxruntime` directly — with SentencePiece for tokenization and NumPy for mean pooling — removes PyTorch from the dependency tree entirely and brings the service in at ~250 MB resident.

Getting there took measuring rather than guessing. The first ONNX build still ran out of memory on a 512 MB container, and the obvious suspect — the model — was not the culprit:

| Stage | Resident |
|---|---|
| Python + NumPy + onnxruntime + FastAPI | 69 MB |
| **+ load `tokenizer.json`** | **319 MB** |
| + ONNX session (the model itself) | 449 MB |

The Hugging Face `tokenizers` library expands XLM-R's 250k-entry vocabulary into ~250 MB of in-memory structures — more than twice what the quantized weights cost. Reading the same vocabulary from the original 5 MB `sentencepiece.bpe.model` instead cuts that to almost nothing. XLM-R layers a fairseq dictionary over SentencePiece (four reserved special tokens, every other id shifted by one), so the service reimplements that mapping; token ids were verified identical to the reference tokenizer across the whole corpus before the swap.

`scikit-learn` is imported lazily inside `/cluster` for the same reason: ~150 MB at import time, and only the offline clustering job needs it.
...
| Inference | FastAPI · ONNX Runtime · SentencePiece · scikit-learn |
